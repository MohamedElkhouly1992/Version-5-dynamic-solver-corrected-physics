...
def _read_csv_fallback(file_or_path) -> pd.DataFrame:
    """Robust CSV reader for uploads and external reference files."""
    return read_csv_robust(file_or_path)
...
def _read_designbuilder_grid_csv(file_or_path) -> pd.DataFrame:
    ...
    """
    last_error = None
    for enc in ["utf-8-sig", "utf-8", "utf-16", "utf-16-le", "utf-16-be", "cp1252", "latin1", "ISO-8859-1"]:
        for skip in [3, 0, 1, 2]:
            try:
                if hasattr(file_or_path, "seek"):
                    file_or_path.seek(0)
                df = pd.read_csv(file_or_path, encoding=enc, skiprows=skip, sep=None, engine="python")
